...
def get_loss(
    ...
    loss = (
        loss_divergence
        + 10.0 * loss_momentum_x
        + 10.0 * loss_momentum_y
        + 10.0 * loss_momentum_z
        + 100.0 * inlet_boundary_loss
        + 1000.0 * other_boundary_loss
    )
    print(
        f"Loss: {loss_divergence.item()}, {loss_momentum_x.item()}, {loss_momentum_y.item()}, {loss_momentum_z.item()}, {inlet_boundary_loss.item()}, {other_boundary_loss.item()}"
    )
    wandb.log(
        {
            ...
            "Inlet Boundary Loss": np.log(inlet_boundary_loss.item()),
            "Other Boundary Loss": np.log(other_boundary_loss.item()),
        }
    )
    return loss
